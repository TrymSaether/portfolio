...
function Dome() {
  ...
  const uniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uTop: { value: new THREE.Color("#04060c") },
      uMid: { value: new THREE.Color("#0d1730") },
      uHorizon: { value: new THREE.Color("#243a5c") },
      uGlow: { value: new THREE.Color("#f3c66b") },
    }),
    [],
  );
      ...
      <shaderMaterial
        ...
        fragmentShader={/* glsl */ `
          ...
          void main() {
            ...
            // Slight directional tint where the moon sits
...
function Stars({ count, radius }: { count: number; radius: number }) {
  ...
  const { positions, sizes } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      // Hemisphere distribution biased toward upper sky
      const u = Math.random();
      const v = Math.random();
      const theta = u * Math.PI * 2;
      const phi = Math.acos(1 - v * 1.4); // 0..1.4 => upper bias
...
function Aurora() {
  ...
  const uniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uColorA: { value: new THREE.Color("#74c0c8") },
      uColorB: { value: new THREE.Color("#a78bf3") },
    }),
    [],
  );
